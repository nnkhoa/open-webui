				...
				<div>
					<div class=" py-0.5 flex w-full justify-between items-center">
						<div class="flex flex-col">
							<div id="artifact-display-mode-label" class="text-xs">
								{$i18n.t('Artifact Display Mode')}
							</div>
							<div class="text-xs text-gray-500 dark:text-gray-400">
								{$i18n.t('Choose where to display HTML/SVG artifacts')}
							</div>
						</div>

						<select
							id="artifact-display-mode-label"
							class="bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-xs rounded-md px-3 py-2 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
							bind:value={$settings.artifactDisplayMode}
							on:change={(e) => {
								saveSettings({ artifactDisplayMode: e.target.value });
							}}
						>
							<option value="sidebar">🎛️ Sidebar Only</option>
							<option value="inline">💬 Inline Only</option>
							<option value="both">🔄 Both (Preview + Sidebar)</option>
						</select>
					</div>
				</div>
			</div>
